...
function AccountingFields({ association, choice, setChoice }: Pick<SocialFieldsProps, "association" | "choice" | "setChoice">) {
  return <>
    <AccountingSection number="1" title="Identificação inicial da associação/cooperativa"><Grid><Field label="Nome completo da associação/cooperativa"><Input name="accounting_association_nome" defaultValue={association?.nome} required /></Field><Field label="CNPJ (se tiver)"><Input name="accounting_association_cnpj" defaultValue={association?.cnpj ?? ""} /></Field><Field label="Município"><Input name="accounting_association_municipio" defaultValue={association?.municipio} required /></Field></Grid></AccountingSection>
    <AccountingSection number="2" title="Gestão e organização"><Grid><Choice name="estatuto_registrado" label="Possui Estatuto registrado na Junta Comercial do Estado?" options={YN} value={choice("estatuto_registrado")} onChange={setChoice} /><Choice name="alvara_funcionamento" label="Possui alvará de funcionamento/localização?" options={YN} value={choice("alvara_funcionamento")} onChange={setChoice} /><Choice name="licenca_ambiental_status" label="Possui licença ambiental ou certificado de dispensa?" options={["Licença ambiental","Certificado de Dispensa","Nenhum"]} value={choice("licenca_ambiental_status", "Nenhum")} onChange={setChoice} /><Choice name="avcb" label="Possui atestado de vistoria do Corpo de Bombeiros Militar de PE?" options={YNK} value={choice("avcb")} onChange={setChoice} /><Choice name="extintores" label="Possui extintores de incêndio?" options={YNK} value={choice("extintores")} onChange={setChoice} /><Choice name="registro_ocb" label="Possui registro no sistema OCB?" options={YNK} value={choice("registro_ocb")} onChange={setChoice} /><Field label="Livros da organização (tirar fotos)" wide><div className="overflow-x-auto rounded-lg border border-border"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-muted"><tr><th className="p-3">Tipo</th><th className="p-3">Implantado</th><th className="p-3">Atualizado</th><th className="p-3">Observação</th></tr></thead><tbody>{ACCOUNTING_BOOKS.map((book) => <tr key={book.key} className="border-t border-border"><td className="p-3 font-medium">{book.label}</td><td className="p-3"><CompactChoice name={`livro_${book.key}_implantado`} options={YNK} value={choice(`livro_${book.key}_implantado`)} onChange={setChoice} /></td><td className="p-3"><CompactChoice name={`livro_${book.key}_atualizado`} options={YNK} value={choice(`livro_${book.key}_atualizado`)} onChange={setChoice} /></td><td className="p-3"><Input name={`livro_${book.key}_observacao`} /></td></tr>)}</tbody></table></div></Field></Grid></AccountingSection>
    <AccountingSection number="3" title="Departamento de pessoal"><Grid><NumberField name="empregados_registrados" label="Quantos empregados registrados?" /><NumberField name="empregados_sem_registro" label="Quantos trabalhadores sem registro?" /><NumberField name="autonomos" label="Quantos trabalhadores autônomos?" /><Choice name="livro_ficha_trabalho" label="Existe livro ou ficha de trabalho? (tirar foto)" options={YN} value={choice("livro_ficha_trabalho")} onChange={setChoice} /><Field label="Se sim, qual?"><Input name="livro_ficha_trabalho_qual" /></Field><Choice name="livro_inspecao_trabalho" label="Existe livro de inspeção do trabalho? (tirar foto)" options={YN} value={choice("livro_inspecao_trabalho")} onChange={setChoice} /><Choice name="filiacao_sindical" label="A organização é filiada a algum sindicato?" options={YN} value={choice("filiacao_sindical")} onChange={setChoice} /><Field label="Se sim, qual sindicato?"><Input name="filiacao_sindical_qual" /></Field><Choice name="contrato_sst" label="Possui contrato para acompanhamento de SST?" options={YNK} value={choice("contrato_sst")} onChange={setChoice} /><Field label="Empresa ou responsável técnico"><Input name="contrato_sst_responsavel" /></Field><Choice name="controle_frequencia" label="Tem controle de frequência dos empregados?" options={YNK} value={choice("controle_frequencia")} onChange={setChoice} /><Choice name="controle_frequencia_tipo" label="Se sim, qual tipo?" options={["Livro/ficha","Eletrônico","Outro","Não se aplica"]} value={choice("controle_frequencia_tipo", "Não se aplica")} onChange={setChoice} /></Grid></AccountingSection>
    <AccountingSection number="4" title="Diagnóstico contábil e fiscal"><Grid><Choice name="possui_contador" label="A organização possui contador?" options={YNK} value={choice("possui_contador")} onChange={setChoice} /><Choice name="contador_tipo" label="Se sim, qual o tipo de contrato?" options={["Terceirizado","Voluntário","Empregado","Cooperado","Outro tipo","Não tem contador"]} value={choice("contador_tipo", "Não tem contador")} onChange={setChoice} /><Field label="Nome do contador"><Input name="contador_nome" /></Field><Field label="Telefone/WhatsApp"><Input name="contador_telefone" type="tel" /></Field><Field label="E-mail"><Input name="contador_email" type="email" /></Field><Choice name="contabilidade_regular" label="Possui contabilidade regular?" options={YNK} value={choice("contabilidade_regular")} onChange={setChoice} /><Choice name="possui_conta_bancaria" label="Possui conta bancária?" options={YNK} value={choice("possui_conta_bancaria")} onChange={setChoice} /><Choice name="possui_maquineta" label="Trabalha com maquineta de cartão?" options={YNK} value={choice("possui_maquineta")} onChange={setChoice} /><Choice name="emite_notas_fiscais" label="Realiza emissão de notas fiscais?" options={YNK} value={choice("emite_notas_fiscais")} onChange={setChoice} /><Choice name="controle_estoque" label="Faz controle de estoque?" options={YNK} value={choice("controle_estoque")} onChange={setChoice} /><Choice name="sistema_financeiro" label="Usa sistema para controle financeiro?" options={YNK} value={choice("sistema_financeiro")} onChange={setChoice} /><Field label="Se sim, qual sistema?"><Input name="sistema_financeiro_qual" /></Field><NumberField name="ano_ultimo_balanco" label="Ano do último balanço apresentado" /></Grid></AccountingSection>
    <AccountingSection number="5" title="Remuneração e divisão dos resultados"><Grid><Field label="Como é feita a divisão do dinheiro entre os cooperados?" wide><Textarea name="divisao_resultados_procedimento" /></Field><Field label="A divisão é igual para todos ou depende da produção?" wide><Textarea name="divisao_resultados_criterio" /></Field><Choice name="pagamento_fixo_mensal" label="Existe pagamento fixo mensal?" options={YN} value={choice("pagamento_fixo_mensal")} onChange={setChoice} /><NumberField name="renda_media_cooperado" label="Renda média mensal de um cooperado" step="0.01" /></Grid></AccountingSection>
    <AccountingSection number="6" title="Avaliação final" subtitle="Preenchimento pelo consultor após a entrevista"><Grid><Choice name="classificacao_contabil" label="Situação da cooperativa" options={["Regular","Parcialmente regular","Irregular"]} value={choice("classificacao_contabil", "Irregular")} onChange={setChoice} /><Field label="Principais pendências identificadas" wide><Textarea name="pendencias_contabeis" /></Field><Field label="Checklist das fotos / comprovação da visita" wide><div className="grid gap-3 rounded-lg bg-muted p-4 sm:grid-cols-2"><Evidence name="evidencia_frente_confirmada" label="Frente da cooperativa (placa de identificação)" /><Evidence name="evidencia_reuniao_confirmada" label="Momento da reunião com todos" /><Evidence name="evidencia_entrevista_confirmada" label="Momento da entrevista" /><Evidence name="evidencia_administrativo_confirmada" label="Sala de administração, financeiro e almoxarifado" /></div></Field></Grid></AccountingSection>
  </>;
}

function AccountingSection({ number, title, subtitle, children }: { number: string; title: string; subtitle?: string; children: ReactNode }) { return <section className="rounded-xl border border-warning/50 bg-card p-5 shadow-card md:p-7"><div className="mb-6 flex gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-warning font-bold text-warning-foreground">{number}</span><div><h2 className="text-lg font-bold uppercase tracking-tight">{title}</h2>{subtitle && <p className="text-sm text-muted-foreground">{subtitle}</p>}</div></div>{children}</section>; }
function CompactChoice({ name, options, value, onChange }: { name: string; options: string[]; value: string; onChange: (name: string, value: string) => void }) { return <Select value={value} onValueChange={(next) => onChange(name, next)}><SelectTrigger className="min-w-36"><SelectValue /></SelectTrigger><SelectContent>{options.map((option) => <SelectItem key={option} value={option}>{option}</SelectItem>)}</SelectContent></Select>; }
function Evidence({ name, label }: { name: string; label: string }) { return <label className="flex items-start gap-3 text-sm"><Checkbox name={name} required /><span>{label}</span></label>; }
