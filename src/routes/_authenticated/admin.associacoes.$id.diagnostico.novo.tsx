...
function LegalFields({ association, choice, setChoice }: Pick<SocialFieldsProps, "association" | "choice" | "setChoice">) {
  return <>
    <LegalSection number="1" title="Identificação inicial da associação/cooperativa"><Grid><Field label="Nome completo da associação/cooperativa"><Input name="legal_association_nome" defaultValue={association?.nome} required /></Field><Field label="CNPJ (se tiver)"><Input name="legal_association_cnpj" defaultValue={association?.cnpj ?? ""} /></Field><Field label="Município"><Input name="legal_association_municipio" defaultValue={association?.municipio} required /></Field></Grid></LegalSection>
    <LegalSection number="2" title="Gestão e organização"><Grid><Choice name="diretoria_conselho" label="Existe diretoria ou conselho de administração?" options={YN} value={choice("diretoria_conselho")} onChange={setChoice} /><Field label="Se sim, nomes dos principais responsáveis"><Textarea name="diretoria_nomes" /></Field><Choice name="mandato_em_dia" label="A diretoria está com o mandato em dia?" options={YN} value={choice("mandato_em_dia")} onChange={setChoice} /><Choice name="conselho_fiscal" label="Existe Conselho Fiscal?" options={YNK} value={choice("conselho_fiscal")} onChange={setChoice} /><Choice name="cargos_por_eleicao" label="Os cargos foram definidos por eleição?" options={YNK} value={choice("cargos_por_eleicao")} onChange={setChoice} /><Field label="Quando foi a última eleição da cooperativa?"><Input name="data_ultima_eleicao" type="date" /></Field><Choice name="ata_registrada_cartorio" label="A ata foi registrada em cartório?" options={YN} value={choice("ata_registrada_cartorio")} onChange={setChoice} /><Choice name="realiza_assembleias" label="Realiza reuniões ou assembleias com os cooperados?" options={YNK} value={choice("realiza_assembleias")} onChange={setChoice} /><Field label="Com que frequência essas reuniões acontecem?"><Input name="frequencia_assembleias" /></Field><Choice name="possui_registro_atas" label="Existe registro dessas reuniões (atas)?" options={YN} value={choice("possui_registro_atas")} onChange={setChoice} /></Grid></LegalSection>
    <LegalSection number="3" title="Apoio técnico e administrativo"><Grid><Choice name="assessoria_juridica" label="Possui assessoria jurídica ou advogado?" options={YN} value={choice("assessoria_juridica")} onChange={setChoice} /><Choice name="apoio_instituicoes" label="Recebe apoio de alguma instituição?" options={YN} value={choice("apoio_instituicoes")} onChange={setChoice} /><Field label="Se sim, quais instituições?" wide><Textarea name="apoio_instituicoes_quais" /></Field><Choice name="processos_judiciais" label="Possui processos judiciais (trabalhista ou cível)?" options={YN} value={choice("processos_judiciais")} onChange={setChoice} /><Field label="Se sim, quais processos?" wide><Textarea name="processos_judiciais_quais" /></Field></Grid></LegalSection>
    <LegalSection number="4" title="Quadro de cooperados"><Grid><Choice name="todos_sao_cooperados" label="Todos que trabalham são cooperados?" options={YN} value={choice("todos_sao_cooperados")} onChange={setChoice} /><Choice name="lista_cooperados_atualizada" label="Existe cadastro ou lista atualizada dos cooperados? (pedir foto)" options={YN} value={choice("lista_cooperados_atualizada")} onChange={setChoice} /><Choice name="lista_nao_cooperados_atualizada" label="Existe lista atualizada dos não cooperados? (pedir foto)" options={YN} value={choice("lista_nao_cooperados_atualizada")} onChange={setChoice} /><Choice name="regras_entrada" label="Existem regras para entrada de novos cooperados?" options={YN} value={choice("regras_entrada")} onChange={setChoice} /><Choice name="regras_saida_exclusao" label="Existem regras para saída ou exclusão de cooperados?" options={YN} value={choice("regras_saida_exclusao")} onChange={setChoice} /></Grid></LegalSection>
    <LegalSection number="5" title="Forma de trabalho e organização das atividades"><Grid><Field label="Como funciona o trabalho no dia a dia? (coleta, triagem, venda etc.)" wide><Textarea name="fluxo_trabalho_diario" /></Field><Choice name="divisao_tarefas" label="Existe divisão de tarefas entre os cooperados?" options={YN} value={choice("divisao_tarefas")} onChange={setChoice} /><Choice name="coordenacao_gerencia" label="Existe alguém que coordena ou gerencia as atividades?" options={YN} value={choice("coordenacao_gerencia")} onChange={setChoice} /><Choice name="controle_jornada" label="Existe controle de horário ou jornada de trabalho?" options={YN} value={choice("controle_jornada")} onChange={setChoice} /></Grid></LegalSection>
    <LegalSection number="6" title="Principais dificuldades"><Grid><Field label="Quais são hoje os principais problemas da cooperativa?" wide><Textarea name="problemas_juridicos_atuais" /></Field><Field label="O que mais precisa melhorar atualmente?" wide><Textarea name="melhorias_juridicas_necessarias" /></Field></Grid></LegalSection>
    <LegalSection number="7" title="Relação com o poder público"><Grid><Choice name="contrato_tipo" label="Possui contrato remunerado com prefeitura ou outra instituição?" options={["Sim, prefeitura","Sim, outra instituição pública","Sim, instituição privada","Não"]} value={choice("contrato_tipo", "Não")} onChange={(name, value) => { setChoice(name, value); setChoice("contrato_remunerado", value === "Não" ? "Não" : "Sim"); }} /><Field label="Se sim, quais são as públicas?"><Textarea name="contrato_instituicoes_publicas" /></Field><Field label="Se sim, quais são as privadas?"><Textarea name="contrato_instituicoes_privadas" /></Field><Choice name="participa_coleta_seletiva_municipal" label="Participa da coleta seletiva do município?" options={YN} value={choice("participa_coleta_seletiva_municipal")} onChange={setChoice} /><Choice name="apoio_poder_publico" label="Recebe apoio do poder público (estrutura, transporte, equipamentos)?" options={YNK} value={choice("apoio_poder_publico")} onChange={setChoice} /></Grid></LegalSection>
    <LegalSection number="8" title="Avaliação final" subtitle="Preenchimento pelo consultor após a entrevista"><Grid><Choice name="classificacao_juridica" label="Situação da cooperativa" options={["Regular","Parcialmente regular","Irregular"]} value={choice("classificacao_juridica", "Irregular")} onChange={setChoice} /><Field label="Principais pendências identificadas" wide><Textarea name="pendencias_juridicas" /></Field><Field label="Orientações ao agente de campo" wide><div className="space-y-3 rounded-lg bg-muted p-4 text-sm leading-relaxed text-muted-foreground"><p><strong>1.</strong> Antes do envio, o declarante deve autorizar expressamente o tratamento dos dados e declarar a veracidade das informações.</p><p><strong>2.</strong> Antes da entrevista, apresente o projeto, sua finalidade e benefícios. Comece com perguntas gerais, aproveite informações já fornecidas e evite repetições que causem desgaste.</p></div></Field></Grid></LegalSection>
  </>;
}

function LegalSection({ number, title, subtitle, children }: { number: string; title: string; subtitle?: string; children: ReactNode }) { return <section className="rounded-xl border border-primary/30 bg-card p-5 shadow-card md:p-7"><div className="mb-6 flex gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-primary font-bold text-primary-foreground">{number}</span><div><h2 className="text-lg font-bold uppercase tracking-tight">{title}</h2>{subtitle && <p className="text-sm text-muted-foreground">{subtitle}</p>}</div></div>{children}</section>; }
