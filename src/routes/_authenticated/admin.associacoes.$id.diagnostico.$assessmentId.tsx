...
function AssessmentDetails() {
  ...
  return <AdminShell>
    ...
    <section className="mb-7 grid gap-5 border-b border-border pb-7 lg:grid-cols-[1fr_280px]">
      ...
      <div className="rounded-xl bg-primary p-5 text-primary-foreground"><p className="text-xs font-bold uppercase tracking-wider opacity-80">Índice de regularidade</p><div className="mt-1 flex items-end gap-2"><strong className="text-4xl tabular-nums">{Number(a.regularity_index).toLocaleString("pt-BR")}%</strong><span className="pb-1 text-sm opacity-80">{a.regularity_compliant_count}/{a.regularity_total_count} critérios</span></div><progress className="mt-3 h-2 w-full accent-current" max="100" value={Number(a.regularity_index)} aria-label="Índice de regularidade" /></div>
    </section>
