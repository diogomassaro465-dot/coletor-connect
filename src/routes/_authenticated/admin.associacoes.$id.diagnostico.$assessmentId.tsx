...
function AssessmentDetails() {
  ...
  async function exportPdf() {
    ...
    const lines = [
      ...
      "", "Critérios de regularidade", `Mandato em dia: ${yes(data.assessment.mandato_em_dia)}`, `Ata registrada: ${yes(data.assessment.ata_registrada_cartorio)}`,
      ...
      "", "Resumo operacional", `Associados informados: ${data.assessment.homens + data.assessment.mulheres}`, `Materiais: ${data.assessment.materiais_coletados.join(", ") || "Não informado"}`, `Volumetria: ${data.assessment.volumetria_toneladas_mes ?? 0} t/mês`,
    ];
  ...
  return <AdminShell>
    ...
    <div className="mb-8 flex flex-wrap items-start justify-between gap-4 border-b border-border pb-7"><div><Badge variant="outline">{STATUS[a.status]}</Badge><h1 className="mt-3 text-3xl font-bold">{association.nome}</h1><p className="mt-1 text-muted-foreground">Visita de {formatDate(a.data_visita)} · {a.consultant_name}</p></div><div className="flex gap-2"><Button variant="outline" onClick={exportExcel}><FileSpreadsheet className="size-4" /> Excel integral</Button><Button onClick={exportPdf}><Download className="size-4" /> Gerar PDF</Button></div></div>
    <Tabs defaultValue="resumo"><TabsList className="h-auto flex-wrap"><TabsTrigger value="resumo">Resumo</TabsTrigger><TabsTrigger value="precos">Compradores e preços</TabsTrigger><TabsTrigger value="equipamentos">Equipamentos</TabsTrigger><TabsTrigger value="livros">Livros</TabsTrigger><TabsTrigger value="evidencias">Evidências</TabsTrigger></TabsList>
      <TabsContent value="resumo"><div className="mt-5 grid gap-5 md:grid-cols-3"><Summary title="Social" rows={[["Homens",a.homens],["Mulheres",a.mulheres],["INSS",a.contribuicao_inss],["CadÚnico",a.inscritos_cadunico],["EPIs",a.uso_epis],["Volumetria",`${a.volumetria_toneladas_mes ?? 0} t/mês`]]} /><Summary title="Jurídico" rows={[["Mandato em dia",yes(a.mandato_em_dia)],["Ata registrada",yes(a.ata_registrada_cartorio)],["Conselho fiscal",a.conselho_fiscal],["Assessoria jurídica",yes(a.assessoria_juridica)],["Contrato remunerado",yes(a.contrato_remunerado)]]} /><Summary title="Contábil" rows={[["Estatuto",yes(a.estatuto_registrado)],["Alvará",yes(a.alvara_funcionamento)],["Licença",a.licenca_ambiental_status],["Contabilidade",a.contabilidade_regular],["Notas fiscais",a.emite_notas_fiscais],["Estoque",a.controle_estoque]]} /></div></TabsContent>
